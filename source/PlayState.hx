...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (songLowercase)
		{
			...
			case 'what-even':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/what-even/what-evenDialogue'));
			case 'peaches':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/peaches/peachesDialogue'));
			case 'savannah':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/savannah/savannahDialogue'));		
			case 'no-exceptions':
				if (poop == 'No-Exceptions-easy'){
					dialogue = CoolUtil.coolTextFile(Paths.txt('data/no-exceptions/no-exceptionsEasyDialogue'));
				}
				else{
					if (poop == 'No-Exceptions-hard'){
						dialogue = CoolUtil.coolTextFile(Paths.txt('data/no-exceptions/no-exceptionsDialogue'));}
					else{
						dialogue = CoolUtil.coolTextFile(Paths.txt('data/no-exceptions/no-exceptionsNormalDialogue'));
					}
				}
			case 'they-leaving':
				dialogue = CoolUtil.coolTextFile(Paths.txt('data/they-leaving/they-leavingDialogue'));	
		}
				...
				case 'bar':
					{
						defaultCamZoom = 0.6;
						curStage = 'bar';
						var bg:FlxSprite = new FlxSprite(-700, -300).loadGraphic(Paths.image('boom/bar_stage', 'shared'));
						bg.antialiasing = true;
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		if (StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'roses' || StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'thorns' || StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'what-even' || StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'peaches' || StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'savannah' || StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'no-exceptions' || StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'they-leaving')
		{
			remove(black);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			switch (noteTypeCheck)
			{
				...
				case 'NoE':
					babyArrow.frames = Paths.getSparrowAtlas('NOTE_assets_NoE');
					for (j in 0...4)
					{
	...
	function endSong():Void
	{
		...
		if (offsetTesting)
		{
			FlxG.sound.playMusic(Paths.music('downtownNoTown'));
			offsetTesting = false;
